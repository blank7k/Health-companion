...
export default function Analytics() {
    ...
    const getAnalyticsData = () => {
        // No condition_status in mockPatients, so fake some data for demo
        const conditionData = [
        ...
        const dischargeStatusData = [
            { name: 'Ready', value: patients.filter(p => p.dischargeStatus === 'ready').length },
            { name: 'Pending', value: patients.filter(p => p.dischargeStatus === 'pending').length },
            { name: 'Delayed', value: patients.filter(p => p.dischargeStatus === 'delayed').length }
        ];

        // Generate trend data for the past 7 days (simulate with random/fake data)
    ...
    const totalDischarges = patients.filter(p => p.dischargeStatus === 'ready').length;
    const avgLengthOfStay = patients.length > 0 ?
        Math.round(patients.reduce((sum, p) => {
            if (p.admissionDate) {
                const admission = new Date(p.admissionDate);
                const discharge = new Date();
                return sum + Math.abs(discharge - admission) / (1000 * 60 * 60 * 24);
            }
            return sum;
        }, 0) / patients.length) : 0;
    ...
    const metrics = [
        ...
        {
            title: "Ready Rate",
            value: `${patients.length > 0 ? Math.round((totalDischarges / patients.length) * 100) : 0} %`,
            change: "+5%",
            icon: TrendingUp,
            color: "emerald"
        }
    ...
    return (
        <div className="p-6 space-y-8 bg-black min-h-screen">
            <div className="max-w-7xl mx-auto">
                {/* Header */}
                <div className="mb-8">
                    <h1 className="text-3xl font-bold text-white mb-2 drop-shadow-lg">Analytics Dashboard</h1>
                    <p className="text-blue-300">Comprehensive insights into patient discharge patterns</p>
                </div>
